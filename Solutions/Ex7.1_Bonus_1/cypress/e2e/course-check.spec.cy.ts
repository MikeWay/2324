describe('Course Check - Ex7.2 Start', () => {
  ...
  it('should have ngModel bindings app-payment form', () => {
    cy.visit('/buy');
    cy.get('app-payment form input[ng-reflect-model]').should('be.visible');
  });  

  // End positive tests for Ex7.2
  // Following tests are to verify that code has not been completed!   

  it('should not yet have formControlName bindings in app-payment form', () => {
    cy.visit('/buy');
    cy.get('app-payment form input[formControlName]').should('not.exist');
  });
