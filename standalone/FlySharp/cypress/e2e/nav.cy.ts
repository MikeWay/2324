describe('Navigation between pages of FlySharp App', () => {
    ...
    it('Visits the My Flights page when the link is clicked', () => {
      cy.visit('/');
      cy.get('a[href="/myflights"]').contains('My Flights').should('exist').click(); // Click the Buy Flights Link
      cy.get('table').should('exist');
    });  

    it('Visits the Accounts page when the link is clicked', () => {
      cy.visit('/');
      cy.get('a[href="/account"]').contains('Account').should('exist').click(); // Click the Buy Flights Link
